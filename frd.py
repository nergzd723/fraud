    ...
    def EncodeAndTransmit(self, data):
        for _ in range(10):
            self.streamout.write(
                self.volume*self.CreateSample(self.freqStart, self.fs, 0.1))
            self.streamout.write(
                0*self.CreateSample(self.freqStart, self.fs, self.speed+0.1))
        r = open(data, "r")
        bdata = r.read()
        bita = self.tobits(bdata)
        ctr = 0
        for ind, bit in enumerate(bita):
            if ctr == 8:
                ctr = 0
                self.streamout.write(
                    self.volume*self.CreateSample(self.freqBYTE, self.fs, 0.55))
            ctr += 1
            self.Transmit(bit)
            try:
                if bita[ind+1] == bit:
                ...
                pass
        self.streamout.write(
            self.volume*self.CreateSample(self.freqEND, self.fs, self.speed+0.6))
